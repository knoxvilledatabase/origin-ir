...
This is not about memory safety. Rust already solved that. This is about **arithmetic provenance**: what happens when a value hits a boundary during computation, and how every operation downstream knows about it without checking.
...
- 8 zero-checks in a linear solver dissolved to 1. The compiler asks once, at the division.
...
The number zero has a rich history which collapsed multiple meanings into one symbol. Every null check, every NaN propagation, every division-by-zero trap is a consequence of that collapse.
...
This is not about pointers or memory lifetimes. This is about what happens when `7.0 / 0.0` executes. Traditional compilers call it undefined, produce infinity, or trap. Origin-ir calls it `container(7.0)`: the boundary was crossed and the last value is preserved. When `0.0 / 0.0` executes, that's `origin`: nothing to retrieve. The sort propagates forward. Every downstream operation knows without checking.
...
| Linear solver (Cramer's rule) | Do redundant zero-checks dissolve? | 8 checks to 1. The compiler asks once, at the division. |
...
When origin enters at the first layer norm, 127 of 193 operations fold. The traditional compiler computes NaN times weight plus bias times weight through every layer. Origin-ir never emits those instructions.

### The linear solver result

Cramer's rule for a 2x2 system. 17 operations.
...
Origin-ir: 15 operations resolve to contents by construction. 2 runtime checks at the two divisions by determinant. The compiler asks "is the determinant zero?" once, at the division. The answer propagates. Every downstream operation inherits the sort without checking again.
...
The IR eliminates roughly 90% of sort checks at compile time. The [ISA extension](https://github.com/knoxvilledatabase/original-arithmetic/tree/main/origin-isa) makes the residual 10% cheap in hardware: 2-bit register reads instead of comparison and branch.
...
Three constructors. The sort is in the type. (Rust convention capitalizes enum variants. The spec uses lowercase throughout.)
